...
class JobChain:
    ...
    def create_job_map(self, job):
        if isinstance(job, Dict):
        ...
        elif self._is_job_abc_like(job):  # Use more flexible check
            ...
            # Check each job with the more flexible approach
            if not all(self._is_job_abc_like(j) for j in job):
                raise TypeError("All items in job collection must be JobABC-compatible instances")
            for j in job:
                if self._is_job_abc_like(j):
                    if j.name in self.job_map:
                        raise ValueError(f"Duplicate job name found: {j.name}")
                    self.job_map[j.name] = j
                else:
                    raise TypeError("Items in job collection must be JobABC-compatible instances")
        else:
            raise TypeError("job must be either Dict[str, Any], JobABC-compatible instance, or Collection of JobABC-compatible instances")
